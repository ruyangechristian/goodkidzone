...
export default function DashboardPage() {
  ...
  useEffect(()=>{
    ...
    fetch('/api/user')
      .then(res=>res.json())
      .then(data=>{
        if(data.authenticated) { setUser(data.user); setIsLoading(false) }
        else { localStorage.removeItem('auth_token'); router.push('/signin') }
      })
  ...
  if(isLoading) return (
    <><Header /><main className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-center"><div className="inline-block animate-spin rounded-full h-12 w-12 border-4 border-primary border-t-transparent"></div><p className="mt-4 text-muted-foreground">{t('common.loading')}</p></div>
    </main></>
  )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-gradient-to-r from-primary/10 via-accent/10 to-primary/5 rounded-xl p-8 md:p-12 mb-12 border border-muted">
          <div className="flex justify-between items-start md:items-center gap-6">
            ...
            <Button onClick={handleLogout} variant="outline" className="flex items-center gap-2"><LogOut size={18} />{t('nav.signOut')}</Button>
          </div>
        </div>
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-foreground mb-6">{t('dashboard.continueLearn')}</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {[{icon:Gamepad2,title:t('home.feature1Title'),desc:t('home.feature1Desc'),href:'/games',color:'from-primary/20 to-primary/5'},
              {icon:Video,title:t('home.feature2Title'),desc:t('home.feature2Desc'),href:'/videos',color:'from-accent/20 to-accent/5'},
